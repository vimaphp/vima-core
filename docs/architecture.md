...
## Hybrid RBAC + ABAC
Vima allows you to mix both systems. A `can()` check can first verify if a user has a permission via a role (RBAC) and then refine that check with a context-aware policy (ABAC).
